    ...
    private static TemporalAccessor adjust(final TemporalAccessor temporalAccessor, DateTimeFormatter dateTimeFormatter) {
        ...
        if (zone != null) {
            if (temporalAccessor.isSupported(ChronoField.INSTANT_SECONDS)) {
                return ((Chronology) ZoneId$$ExternalSyntheticBackport0.m(chronology3, IsoChronology.INSTANCE)).mo128zonedDateTime(Instant.from(temporalAccessor), zone);
            }
        ...
        if (chronology != null) {
            if (temporalAccessor.isSupported(ChronoField.EPOCH_DAY)) {
                chronoLocalDate = chronology3.mo153date(temporalAccessor);
                ...
                for (ChronoField chronoField2 : ChronoField.values()) {
                    if (chronoField2.isDateBased() && temporalAccessor.isSupported(chronoField2)) {
                        ...
                        StringBuilder sb2 = new StringBuilder(valueOf3.length() + ScriptIntrinsicBLAS.LEFT + valueOf4.length());
        ...
        return new TemporalAccessor() { // from class: j$.time.format.DateTimePrintContext.1
            ...
            public Object query(TemporalQuery temporalQuery) {
                return temporalQuery == TemporalQueries.chronology() ? chronology3 : temporalQuery == TemporalQueries.zoneId() ? zoneId : temporalQuery == TemporalQueries.precision() ? temporalAccessor.query(temporalQuery) : temporalQuery.mo183queryFrom(this);
            }
