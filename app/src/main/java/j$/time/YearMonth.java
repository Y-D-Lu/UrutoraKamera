...
public final class YearMonth implements Temporal, TemporalAdjuster, Comparable, Serializable {
    ...
    public Temporal adjustInto(Temporal temporal) {
        if (Chronology.CC.from(temporal).equals(IsoChronology.INSTANCE)) {
            return temporal.mo161with(ChronoField.PROLEPTIC_MONTH, getProlepticMonth());
        }
    ...
    /* renamed from: minus */
    public YearMonth mo156minus(long j, TemporalUnit temporalUnit) {
        return j == Long.MIN_VALUE ? mo157plus(Long.MAX_VALUE, temporalUnit).mo157plus(1L, temporalUnit) : mo157plus(-j, temporalUnit);
    }

    @Override // j$.time.temporal.Temporal
    /* renamed from: plus */
    public YearMonth mo157plus(long j, TemporalUnit temporalUnit) {
        long m;
        long m2;
        long m3;
    ...
    @Override // j$.time.temporal.Temporal
    /* renamed from: with */
    public YearMonth mo162with(TemporalAdjuster temporalAdjuster) {
        return (YearMonth) temporalAdjuster.adjustInto(this);
    }

    @Override // j$.time.temporal.Temporal
    /* renamed from: with */
    public YearMonth mo161with(TemporalField temporalField, long j) {
        if (temporalField instanceof ChronoField) {
