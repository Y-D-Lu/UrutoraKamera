...
public final class Year implements Temporal, TemporalAdjuster, Comparable, Serializable {
    ...
    public Temporal adjustInto(Temporal temporal) {
        if (Chronology.CC.from(temporal).equals(IsoChronology.INSTANCE)) {
            return temporal.mo161with(ChronoField.YEAR, this.year);
        }
    ...
    @Override // j$.time.temporal.Temporal
    /* renamed from: with */
    public Year mo162with(TemporalAdjuster temporalAdjuster) {
        return (Year) temporalAdjuster.adjustInto(this);
    }

    @Override // j$.time.temporal.Temporal
    /* renamed from: with */
    public Year mo161with(TemporalField temporalField, long j) {
        if (temporalField instanceof ChronoField) {
