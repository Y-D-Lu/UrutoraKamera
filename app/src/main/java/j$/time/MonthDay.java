...
public final class MonthDay implements TemporalAccessor, TemporalAdjuster, Comparable, Serializable {
    ...
    public Temporal adjustInto(Temporal temporal) {
        if (Chronology.CC.from(temporal).equals(IsoChronology.INSTANCE)) {
            Temporal mo161with = temporal.mo161with(ChronoField.MONTH_OF_YEAR, this.month);
            ChronoField chronoField = ChronoField.DAY_OF_MONTH;
            return mo161with.mo161with(chronoField, Math.min(mo161with.range(chronoField).getMaximum(), this.day));
        }
